...
describe('Client', () => {
  ...
  beforeEach(() => {
    serverConnectStub = jest.spyOn(Connection.prototype, 'connect').mockImplementation(() => {});
  });
  ...
  describe('#constructor', () => {
    ...
    beforeEach(() => {
      initStub = jest.spyOn(Client.prototype, 'init').mockImplementation(() => {});
      connectStub = jest.spyOn(Client.prototype, 'connect').mockImplementation(() => {});
    });
  ...
  describe('#connect', () => {
    ...
    it('connect params', () => {
      ...
      expect(client.server).toBeTruthy();
      expect(client.params.host).toEqual(client.server.host);
      expect(client.params.port).toEqual(client.server.port);
    });
    ...
    it('proxy connection events', () => {
      ...
      expect(onSpy).toHaveBeenNthCalledWith(3, 'error.network', expect.any(Function));
      expect(onSpy).toHaveBeenNthCalledWith(4, 'error.protocol', expect.any(Function));
      expect(onSpy).toHaveBeenNthCalledWith(5, 'error.server', expect.any(Function));
    });
  });
  ...
  describe('#request', () => {
    ...
    beforeEach(() => {
      ...
      serverRequestStub = jest.spyOn(Connection.prototype, 'request').mockImplementation(() => {});
    });
    ...
    it('build request headers - authed', () => {
      ...
      expect(serverRequestStub).toHaveBeenCalledWith('get', 'url', { $data: 'data' }, expect.any(Function));
    });
    ...
    it('build request headers + authed', () => {
      ...
      expect(serverRequestStub).toHaveBeenCalledWith('get', 'url', {
        $client: 'id',
        $key: 'key',
        $data: 'data',
        $route: {
    ...
    it('build request headers with default data', () => {
      ...
      expect(serverRequestStub).toHaveBeenCalledWith('get', 'url', {$data: null}, expect.any(Function));
    });

    it('handle result $auth', () => {
      const authStub = jest.spyOn(Client.prototype, 'auth').mockImplementation();

      serverRequestStub.mockImplementationOnce((_method, _url, _data, callback) => {
        callback({ $auth: true });
      });
  ...
  describe('#get/put/post/delete', () => {
    ...
    beforeEach(() => {
      requestStub = jest.spyOn(Client.prototype, 'request').mockImplementation(() => {});
    ...
    describe('get request caching behaviour', () => {
      ...
      beforeEach(() => {
        ...
        getCacheStub = jest.spyOn(Cache.prototype, 'get').mockImplementation(() => {
  ...
  describe('#auth', () => {
    ...
    it('request nonce if not provided', () => {
      ...
      expect(serverRequestStub).toHaveBeenCalledWith('auth', expect.any(Function));
    });
  ...
  describe('#sendCachedVersions', () => {
    ...
    beforeEach(() => {
      serverRequestStub = jest.spyOn(Connection.prototype, 'request').mockImplementation((_action, _data, callback) => {
        callback();
      });
      getCacheStub = jest.spyOn(Cache.prototype, 'getVersions').mockImplementation(() => {
        return { a: 1 };
      });
