...
describe('Client', () => {
  let serverConnectStub = sinon.stub(Swell.Connection.prototype, 'connect');
  ...
  describe('#constructor', () => {
    let initStub = sinon.stub(Swell.Client.prototype, 'init');
    let connectStub = sinon.stub(Swell.Client.prototype, 'connect');
  ...
  describe('#connect', () => {
    let client;
    let serverSpy = sinon.spy(Swell, 'Connection');

    beforeEach(() => {
      client = new Swell.Client('id', 'key');
      serverSpy.resetHistory();
    });
  ...
  describe('#request', () => {
    ...
    beforeAll(() => {
      ...
      respondSpy = sinon.spy(Swell.Client.prototype, 'respond');
  ...
  describe('#get/put/post/delete', () => {
    ...
    describe('get request caching behaviour', () => {
      ...
      beforeEach(() => {
        client = new Swell.Client('id', 'key', { cache: true });
        client.cache = new Swell.Cache('id');
        client.authed = true;
      ...
      it('returns (error, response) when retrieving from cache', () => {
        let calledBack = false;
  ...
  describe('#createResource', () => {
    let client = new Swell.Client();

    it('return a new collection resource', () => {
      const result = {
        $data: {
          count: 1,
          results: [{}],
    ...
    it('return a new record resource', () => {
      const result = {
        $data: {},
      };
      const resource = Swell.Client.createResource('url', result, client);
