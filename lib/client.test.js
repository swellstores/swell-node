...
describe('Client', () => {
  ...
  describe('#sendCachedVersions', () => {
    ...
    beforeAll(() => {
      serverRequestStub = sinon.stub(Connection.prototype, 'request');
      getCacheStub = sinon.stub(Cache.prototype, 'getVersions').returns({ a: 1 });
    });
    ...
    it('send cached versions to server auth with credentials', () => {
      ...
      assert.strictEqual(getCacheStub.calledOnce, true);
      assert.strictEqual(serverRequestStub.calledOnce, true);
      assert.strictEqual(serverRequestStub.args[0][0], 'cached');
      assert.deepEqual(serverRequestStub.args[0][1], {
        $cached: { a: 1 },
        $push: true,
      });
      serverRequestStub.args[0][2]();
      assert.strictEqual(client.sentVersions, true);
    });
  });
  ...
  describe('#setAuthedEnv', () => {
    ...
    it('sets authed and env flags', () => {
      ...
      assert.strictEqual(client.authed, true);
      assert.strictEqual(client.env, 'test');
      assert.strictEqual(client.cache.env, 'test');
    });
    ...
    it('sets authed and env flags', () => {
      ...
      assert.strictEqual(client.authed, true);
      assert.strictEqual(client.env, undefined);
      assert.strictEqual(client.cache.env, undefined);
    });
  });
