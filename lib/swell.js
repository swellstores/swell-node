/*! swell.js (Version 1.1.4) 2015-11-12 */
...
var Swell = this.Swell = {
    publicUrl: 'https://api.swell.store',
    vaultUrl: 'https://vault.swell.store',
    publicKey: null
};
 ...
 * @param  string key
 */
Swell.setPublicKey = function(key) {
    this.publicKey = key;
 ...
 * @param  string key
 */
Swell.setPublishableKey = function(key) {
    return Swell.setPublicKey(key);
};
 ...
 * @return void
 */
Swell.createToken = function(card, callback) {

    var error = null;
    var param = null
    if (!card) {
        error = 'Card details are missing in `Swell.createToken(card, callback)`';
        param = '';
    }
    if (!callback) {
        error = 'Callback function missing in `Swell.createToken(card, callback)`';
        param = '';
    }
    if (!Swell.card.validateCardNumber(card.number)) {
        error = 'Card number appears to be invalid';
        param = 'number';
    }
    if (card.exp) {
        var exp = Swell.cardExpiry(card.exp);
        card.exp_month = exp.month;
        card.exp_year = exp.year;
    }
    if (!Swell.card.validateExpiry(card.exp_month, card.exp_year)) {
        error = 'Card expiry appears to be invalid';
        param = 'exp_month';
    }
    if (!Swell.card.validateCVC(card.cvc)) {
    ...
    // Get a token from Swell Vault
    Swell.vault().post('/tokens', card, function(result, headers) {
        var response = result || {};
        if (headers.$error) {
...
Swell.card.createToken = function() {
    return Swell.createToken.apply(this, arguments);
};
 ...
 */
Swell.cardExpiry = function(value) {
 ...
 * Determine card type
 */
Swell.cardType = function() {
    return Swell.Stripe.card.cardType.apply(Swell.Stripe, arguments);
};
Swell.card.cardType = function() {
    return Swell.cardType.apply(this, arguments);
};

/**
 * Validate card number
 */
Swell.validateCardNumber = function() {
    return Swell.Stripe.card.validateCardNumber.apply(Swell.Stripe, arguments);
};
Swell.card.validateCardNumber = function() {
    return Swell.validateCardNumber.apply(this, arguments);
};

/**
 * Validate card expiry
 */
Swell.validateExpiry = function() {
    return Swell.Stripe.card.validateExpiry.apply(Swell.Stripe, arguments);
};
Swell.card.validateExpiry = function() {
    return Swell.validateExpiry.apply(this, arguments);
};

/**
 * Validate card CVC code
 */
Swell.validateCVC = function() {
    return Swell.Stripe.card.validateCVC.apply(Swell.Stripe, arguments);
};
Swell.card.validateCVC = function() {
    return Swell.validateCVC.apply(this, arguments);
};
 ...
 * @return Swell.Client
 */
Swell.client = function() {

    if (this._publicClient) {
        return this._publicClient;
    }
    if (!this.publicKey) {
        throw "Error: Public key must be set Swell.setPublicKey()";
    }

    this._publicClient = new Swell.Client(this.publicKey, {hostUrl: this.publicUrl});
...
Swell.vault = function() {
    ...
    this._vaultClient = new Swell.Client(this.publicKey, {hostUrl: this.vaultUrl});
 ...
 * @param  Client client
 */
var Resource = this.Swell.Resource = function(url, result, client) {
...
// Exports
if (typeof exports !== 'undefined') {
    exports.Resource = this.Swell.Resource;
}
...
var util = this.Swell.util;
var Resource = this.Swell.Resource;
 ...
 */
var Record = this.Swell.Record = function(url, result, client, collection) {
...
// Exports
if (typeof exports !== 'undefined') {
    exports.Record = this.Swell.Record;
}
 ...
 * @param  Client client
 */
var Collection = this.Swell.Collection = function(url, result, client) {
...
// Exports for Node
if (typeof exports !== 'undefined') {
    exports.Collection = this.Swell.Collection;
}

}).call(this);
/**
 * Swell API Client for JS
 */
(function() {
 ...
 */
var Client = this.Swell.Client = function(publicKey, options) {
    ...
    this.options = {
        publicKey: publicKey || options.publicKey || this.Swell.publicKey,
        hostUrl: options.hostUrl || this.Swell.publicUrl,
...
Client.prototype.request = function(method, url, data, callback) {
    ...
    data = {
        $jsonp: {
            method: method,
            callback: 'Swell.Client.response'+requestId,
        },
        $data: data,
    ...
    var errorTimeout = setTimeout(function() {
        Swell.Client['response'+requestId]({
            $error: 'Request timed out after '+(self.options.timeout/1000)+' seconds',
            $status: 500
        });
    }, self.options.timeout);

    Swell.Client['response'+requestId] = function(response) {
        clearTimeout(errorTimeout);
        self.response(method, url, response, callback);
        delete Swell.Client['response'+requestId];
...
Client.generateRequestId = function() {
    ...
    window.__swell_client_request_id = window.__swell_client_request_id || 0;
    window.__swell_client_request_id++;
    return window.__swell_client_request_id;
};
...
Client.createResource = function(url, result, client) {
    if (result && result.$data && 'count' in result.$data && result.$data.results) {
        return new Swell.Collection(url, result, client);
    }
    return new Swell.Record(url, result, client);
};
...
var Swell = this.Swell;
var Form = Swell.Form = Swell.form = {};
...
Form._onSubmitCard = function(params, event) {
    ...
    if (params.fields.cardExp) {
        cardExpiry = Swell.cardExpiry(params.fields.cardExp.value);
    } else {
        cardExpiry = {
    ...
    if (!Swell.validateCVC(data.cvc)) {
        fieldsValid = false;
        Form._triggerFieldError(
            params.onError, params.fields.cardCVC, ''
        );
    }
    if (!Swell.validateExpiry(data.exp_month, data.exp_year)) {
        fieldsValid = false;
        if (params.fields.cardExp) {
            Form._triggerFieldError(
    ...
    if (!Swell.validateCardNumber(data.number) || !data.number) {
        fieldsValid = false;
        Form._triggerFieldError(
    ...
    Swell.setPublicKey(params.publicKey);
    Swell.card.createToken(data, function(status, response) {
        if (response.errors) {
...
Form._validateCardParams = function(params) {
    ...
    params.publicKey = params.publicKey || Swell.publicKey;
    if (!params.publicKey) {
