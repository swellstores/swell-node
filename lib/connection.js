...
class Connection extends EventEmitter {
  ...
  requestNext() {
    ...
    if (args) {
      console.log(`--- [swell-node] requestNext ${this.requestBuffer.length} ---\n`, args);
      const requestId = randomUUID();
      const requestArr = args.slice(0, -1);
      const request = JSON.stringify([requestId, requestArr[0], requestArr.slice(1)]);
      console.log('BEFORE SEND', request);
      this.requestCallbacks[requestId] = args[args.length - 1];
      this.stream.write(request + '\n');
      this.requested++;
      console.log(`--- [swell-node] request ${this.requested} sent ---\n`, request);
    }
  }
  ...
  receive(stream, buffer) {
    ...
    for (let i = 0; i < buffer.length; i++) {
      if (buffer[i] === '\n') {
        ...
        this.buffer = [];
        console.log('+++ [swell-node] Data received +++\n', data);
  ...
  receiveResponse(data) {
    ...
    try {
      responseParsed = JSON.parse(data);
    } catch (err) {
      //
    }
    const [reqId, response] = responseParsed;
    console.log('receiveResponse', { reqId, response }, this.requestCallbacks);
    ...
    const responder = this.requestCallbacks[reqId];
